...
//            helper.setText(R.id.tv_xiaofei, item.get_grade().get(0).getStart_amt());
        ...
        if (item.get_grade() != null && item.get_grade().size() > 1) {
            ...
            setVipRes(item.get_grade().get(1).getTitle().contains("普通"), tv);
            helper.setText(R.id.tv_vip, item.get_grade().get(1).getTitle() + "交易手续费：" + item.get_grade().get(1).getFee_per() + "%+" + item.get_grade().get(1).getFixed_fee() + " " + TimeUtils.parseTime(TimeUtils.getStrFormat(item.get_grade().get(1).getDay_time_start(), 4), "HHmm", "HH:mm") + "-" + TimeUtils.parseTime(TimeUtils.getStrFormat(item.get_grade().get(1).getDay_time_end(), 4), "HHmm", "HH:mm") + "交易");
    ...
    private void setVipRes(boolean isVip, TextView textView) {
        Drawable drawable = App.getInstance().getResources().getDrawable(isVip ? R.drawable.ic_service_vip_normal : R.drawable.ic_service_vip);
